...
## Python程序编码规范
遵守编码规范可以提升代码的易读性和易维护性，减少协作冲突，使程序员快速上手，并借助自动化工具提前发现问题，从而直接降低后期修改与扩展的成本。
* 层级缩进
空白在Python中是很重要的，它称为缩进。在逻辑行首的空白（空格和制表符）用来决定逻辑行的缩进层次，从而用来决定语句的分组。这意味着同一层次的语句必须有相同的缩进。每一组这样的语句称为一个块。通常的缩进为4个空格, 在Jupyter Notebook中为一个Tab键。如果想要减少缩进量，可按快捷键“Shift+Tab”。如果要同时调整多行代码的缩进量，可选中要调整的多行代码，按“Tab”键统一增加缩进量，按快捷键“Shift+Tab”统一减少缩进量。
同级别代码必须严格对齐，如果缩进不规范，代码在运行时就会报错。
...
* 注释
注释是对代码的解释和说明，不属于程序的功能，运行时不会执行。Python中支持两种注释，即单行注释和多行注释。Python代码中#号后面的内容全部是注释。此外，一对三个"符号(""")之内包围起来的字符串是多行注释，通常用来作为说明文档。
```
def hello():
  print('Hello, world')
...
* 多行语句 
Python代码中一般一行就是一条语句，但是可以使用斜杠(\)将一条语句分为多行显示。此外，如果语句中包含像括号、中括号和大括号等配对符号，Python会自动将其视为多行语句。
```
>>>total = 1 + 2 +\
...3 + 4 
>>>total
...
## 控制流语句
控制流用来根据运行时的情况调整语句的执行顺序。没有控制流语句，编程语言就不是编程语言。 程序的控制结构是决定程序执行顺序的一系列指令和规则。它通常包括顺序结构、选择结构和循环结构，如图2-1-1所示。每个结构化的程序都可以由这3种结构组成或嵌套而成。这种程序设计方法使程序具有良好的可读性和可维护性。
![alt text](../zx/image/control-structure.png)

在Python中，控制语句主要有三类：条件语句，循环语句和跳转语句。
### 条件语句
