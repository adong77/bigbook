...
其它常见的解剖学或生理学的分类特征有生物体的颜色、体重大小或对某个刺激的反应时间等。如果确定了所有状态之间的相似性的标准，特征数据就可以转换成距离数据。例如，两个基因或蛋白质序列的之间的距离值(D)可以简单通过序列比对来确定，将匹配的位点数(m)除以总的位点数(n)：D = m/n。但是这样的转换可能会使一些序列信息丢失，因为有些核苷酸或氨基酸的替换更容易发生，如哺乳动物的基因组中C被T替换的频率将近C被G或A替换的3倍。所以计算DNA序列的两两距离时，比对中C/T的错配的权重应该与C/G或C/A的权重不一样。
