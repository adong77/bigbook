...
## R语言的数据类型
R中有多种数据类型，包括向量、矩阵、数据框（与数据集类似）以及列表（各种对象的集合）等。注意R是一种区分大小写的解释型语言，R语言的变量要区分大小写！
...
### 矩阵(matrix)
矩阵是由相同数据类型组成的二维数组。矩阵和数组可看成是带维数的向量，矩阵就是一个二维的数值数组。创建矩阵的一个方便的方法是使用matrix函数：
```R
...
### 数据框(data frame)
数据框由一个或几个向量或因子构成，它们必须是等长的，但可以是不同的数据类型。数据框也是二维数组(2D array)，与矩阵不同的地方是数据框中的数据可以是各种类型同时存在，如数值、字符串或者因子等。数据框是生物医学研究中最常用的数据记录格式，数据框中每行代表一个观测研究对象，每列代表研究对象一个特性的实验结果(图2)。
...
### 函数
函数(function)是组织好的，可重复使用的，用来实现单一，或相关功能的代码段。每一个函数的形式为函数名后面紧跟括号()[函数名()],并将参数放入括号中。
一般R的函数需要参数，即传给函数的变量，来进行一些操作，如log10()需要一个数值，计算基于10的log值：
```R
> log10(100)
2
```
使用R内置函数可执行基本的统计任务。当你经常需要对一些数值做同样的运算，而R没有内置的函数，可以自己编写一个函数。R语言的函数形式为：
    function (对象， 选项=)
...
> R语言的函数又分为高级函数与低级函数，高级函数内部嵌套了复杂的低级函数，如plot()是高级绘图函数，有大量的参数可选拔，函数本身会根据参数的数据类型，经过程序内部的函数判别之后，绘制相关类型的图形。
...
```
载入R程序包后，调用程序包内的函数，与R内置的函数调用方法一样。同样，如果不需要程序包可以用detach()函数卸载程序包：
```R
>detach("yeastExpData")
```
